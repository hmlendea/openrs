...
| Packet framing and buffering | `PacketConstruction` and `StreamClass` | Protocol methods only | Connection and action handlers |
...
| Camera scene objects and hit state | `CameraSceneObjectTracker` | Camera scene facade methods | Camera rendering and input |
| Camera model cache and projection state | `Camera` | Existing camera facade methods | Rendering |
| Raw image pixel state | `GameImage` | Focused rasteriser, shape, and blur collaborators | GUI and renderers |
...
| Chat codec buffers | `ChatMessage` | `ChatMessageCodec` through public facade methods | Chat input and packet handling |
