...
namespace OpenRS.Net.Client.Game
{
    public class EngineHandle
    {
        ...
        public EngineHandle(EntityManager entityManager, Camera camera, GraphicsEngine graphics)
        {
            ...
            LoadContent(); // TODO: Move the call outside
        }
...
#warning draw tiles ? -part-1- INITIALIZE
                ...
                for (int j2 = 0; j2 < 96; j2++)
                {
                    for (int i3 = 0; i3 < 96; i3++)
                    {
                        ...
                        Tile tile1 = entityManager.GetTile(getTileGroundOverlayIndex(j2, i3) - 1);

                        if (getTileGroundOverlayIndex(j2, i3) > 0 && tile1.Type == 4)
                        {
                            i4 = 0;
                        }

                        if (getTileGroundOverlayIndex(j2 - 1, i3) > 0 &&
                            entityManager.GetTile(getTileGroundOverlayIndex(j2 - 1, i3) - 1).Type == 4)
                        {
                            i4 = 0;
                        }

                        if (getTileGroundOverlayIndex(j2, i3 - 1) > 0 &&
                            entityManager.GetTile(getTileGroundOverlayIndex(j2, i3 - 1) - 1).Type == 4)
                        {
                            i4 = 0;
                        }

                        if (getTileGroundOverlayIndex(j2 - 1, i3 - 1) > 0 &&
                            entityManager.GetTile(getTileGroundOverlayIndex(j2 - 1, i3 - 1) - 1).Type == 4)
                        {
                            i4 = 0;
                        }

                        Point3D vertLoc = new(j2 * 128, i4, i3 * 128);
...
#warning draw tiles ? -part-2- APPLY TEXTURES TO TILES
                ...
                for (int x1 = 0; x1 < 95; x1++)
                {
                    for (int y1 = 0; y1 < 95; y1++)
                    {
                        ...
                        if (getTileGroundOverlayIndex(x1, y1) > 0)
                        {
                            int tileIndex = getTileGroundOverlayIndex(x1, y1);
                            ...
                            else if (tileType != 2 || getDiagonalWall(x1, y1) > 0 && getDiagonalWall(x1, y1) < 24000)
                            {
                                ...
                                else if (gkd(x1 + 1, y1) != i19 && gkd(x1, y1 + 1) != i19)
                                {
                                    texture1 = texture2;
                                    l14 = 0;
                                }
...
#warning draw tiles ? -part-3- ?????
                ...
                for (int x1 = 1; x1 < 95; x1++)
                {
                    for (int y1 = 1; y1 < 95; y1++)
                    {
                        ...
                        else if (getTileGroundOverlayIndex(x1, y1) == 0 ||
                                 entityManager.GetTile(getTileGroundOverlayIndex(x1, y1) - 1).Type != 3)
                        {
                            if (getTileGroundOverlayIndex(x1, y1 + 1) > 0 &&
                            ...
                            if (getTileGroundOverlayIndex(x1 + 1, y1) > 0 &&
...
#warning adds all tiles
                ...
                for (int y1 = 0; y1 < 95; y1++)
                {
                    ...
                    if (k3 > 0 && (entityManager.GetWallObject(k3 - 1).Unknown == 0 || unknownBool1))
                    {
                        ...
                        if (freshLoad && entityManager.GetWallObject(k3 - 1).Type != 0)
                        {
                            ...
                            if (location.Y > 0)
                            {
                                Point2D loc = new(location.X, location.Y - 1);
                                setTileType(loc, 4);
                            }
                        }
            ...
            {
                camera.addModel(wallObject[height][l2]);
            }
...
#warning loadsection been modified.
        ...
        public void StitchAreaTileColors()
        {
            for (int x = 0; x < 96; x++)
            {
                for (int y = 0; y < 96; y++)
                {
                    if (getTileGroundOverlayIndex(x, y) == 250)
                    {
                        if (x == 47 && getTileGroundOverlayIndex(x + 1, y) != 250 &&
                            getTileGroundOverlayIndex(x + 1, y) != 2)
                        {
                            setTileGroundOverlayHeight(x, y, 9);
                        }
                        else if (y == 47 && getTileGroundOverlayIndex(x, y + 1) != 250 &&
                                 getTileGroundOverlayIndex(x, y + 1) != 2)
                        {
                            setTileGroundOverlayHeight(x, y, 9);
                        }
                        else
        ...
        public void RemoveWallObject(Point2D location, int arg2, int index)
        {
            ...
            Point2D l = new(location.X - 1, location.Y);
        ...
        public int gkd(int x, int y)
        {
            int j1 = getTileGroundOverlayIndex(x, y);
        ...
        public void drawMinimapPixel(int x, int y, int drawOrder, int textureIndex1, int textureIndex2)
        {
            ...
            if (drawOrder == 0)
            {
                ...
                graphics.DrawHorizontalLine(destX, destY + 1, 2, texture1);
                graphics.DrawHorizontalLine(destX, destY + 2, 1, texture1);
                graphics.DrawHorizontalLine(destX + 2, destY + 1, 1, texture2);
            ...
            if (drawOrder == 1)
            {
                ...
                graphics.DrawHorizontalLine(destX + 1, destY + 1, 2, texture2);
                graphics.DrawHorizontalLine(destX + 2, destY + 2, 1, texture2);
                graphics.DrawHorizontalLine(destX, destY + 1, 1, texture1);
                graphics.DrawHorizontalLine(destX, destY + 2, 2, texture1);
            }
        }
        ...
        public int getTileGroundOverlayIndex(int x, int y)
        {
            if (x < 0 || x >= 96 || y < 0 || y >= 96)
            {
                return 0;
